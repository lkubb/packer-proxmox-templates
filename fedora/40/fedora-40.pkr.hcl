locals {
  ...
  iso_url_map = {
    dvd        = "https://download.fedoraproject.org/pub/fedora/linux/releases/40/Server/x86_64/iso/Fedora-Server-dvd-x86_64-40-1.14.iso"
    netinstall = "https://download.fedoraproject.org/pub/fedora/linux/releases/40/Server/x86_64/iso/Fedora-Server-netinst-x86_64-40-1.14.iso"
  }
  ...
  iso_checksum  = coalesce(var.iso_checksum, "file:https://download.fedoraproject.org/pub/fedora/linux/releases/40/Server/x86_64/iso/Fedora-Server-40-1.14-x86_64-CHECKSUM")
  root_password = coalesce(var.root_password, uuidv4())
}

source "proxmox-iso" "fedora40" {
  ...
  template_description = "Fedora 40 template. Built on {{ isotime \"2006-01-02T15:04:05Z\" }}"
  ...
  vm_id   = var.vm_id
  vm_name = "packer-fedora-40-amd64"
}
